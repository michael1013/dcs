...
public class Recharge萌娘契约 extends CommonRecharge {

	private static final long serialVersionUID = -3355539330629548062L;

	public Recharge萌娘契约(String gameid, Map<String, Object> map, String file, Integer sheet) {
		if (MapUtils.isNotEmpty(map)) {
			if (map.size() == 12) {
				...
				this.userid = String.valueOf(map.get(6)).substring(1);
				// this.username = String.valueOf(map.get(3));
				try {
					this.orderid = String.valueOf(map.get(1)).substring(1);
				} catch (Exception e) {
				...
				if (StringUtils.isNoneBlank(this.file)) {
					try {
						this.serverid = this.gameid + '_'
						        + ((Double) Double.parseDouble(String.valueOf(map.get(4)))).longValue();
					} catch (Exception e) {
						e.printStackTrace();
						return;
					}
	...
	public static void main(String[] args) {
		String[] filePaths = { "C:/Users/luke/Documents/logs/萌娘契约" };
		for (String filePath : filePaths) {
			for (int i = 0; i < 1; i++) {
				List<? extends CommonRecharge> list = ParseXlsxFile.read(filePath, "4002001052", Recharge萌娘契约.class,
				        "UTF-8", i);
