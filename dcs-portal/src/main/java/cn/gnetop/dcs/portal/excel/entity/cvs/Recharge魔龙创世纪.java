...
public class Recharge魔龙创世纪 extends CommonRecharge {

	private static final long serialVersionUID = -3355539330629548062L;

	public Recharge魔龙创世纪(String gameid, String value, String file) {
		...
		if (column.length == 6) {
			...
			try {
				this.logTime = DateUtils.formate(
				        DateUtils.parseDate(column[5], "yyyy-MM-dd HH:mm:ss", "yyyy-MM-dd HH:mm:ss.SSSSSSSSS"));
			} catch (ParseException e) {
				e.printStackTrace();
				return;
			}
			this.amount = column[4].replaceAll("\\$", "");
			this.currency = "USD";
			this.isValid = true;
			this.gameid = gameid;
	...
	public static void main(String[] args) {
		String[] filePaths = { "C:/Users/luke/Documents/logs/魔龙创世纪" };
		for (String filePath : filePaths) {
			List<? extends CommonRecharge> list = ParseCsvFile.read(filePath, "4002001020", Recharge魔龙创世纪.class);
