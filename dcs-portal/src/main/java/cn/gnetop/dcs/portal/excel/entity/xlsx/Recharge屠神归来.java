...
public class Recharge屠神归来 extends CommonRecharge {

	private static final long serialVersionUID = -3355539330629548062L;

	public Recharge屠神归来(String gameid, Map<String, Object> map, String file, Integer sheet) {
		if (MapUtils.isNotEmpty(map)) {
			if (map.size() == 22) {
				...
				this.username = String.valueOf(map.get(21));
				try {
					this.orderid = String.valueOf(map.get(11));
				} catch (Exception e) {
				...
				this.channel = String.valueOf(map.get(14));
				if (StringUtils.isNoneBlank(this.file)) {
					try {
						this.serverid = this.gameid + '_' + map.get(20);
					} catch (Exception e) {
						e.printStackTrace();
						return;
					}
	...
	public static void main(String[] args) {
		String[] filePaths = { "D:/My Document/sdk/logs/文件夹/屠神归来-20160919/order_8月24-9月19.xls" };
		for (String filePath : filePaths) {
			for (int i = 0; i < 1; i++) {
				List<? extends CommonRecharge> list = ParseXlsxFile.read(filePath, "4002001030", Recharge屠神归来.class,
				        "UTF-8", i);
