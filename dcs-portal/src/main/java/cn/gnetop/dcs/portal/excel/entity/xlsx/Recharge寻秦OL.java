...
public class Recharge寻秦OL extends CommonRecharge {

	private static final long serialVersionUID = -3355539330629548062L;

	public Recharge寻秦OL(String gameid, Map<String, Object> map, String file, Integer sheet) {
		if (MapUtils.isNotEmpty(map)) {
			if (map.size() == 11) {
				...
				try {
					this.orderid = String.valueOf(map.get(1));
				} catch (Exception e) {
	...
	public static void main(String[] args) {
		String[] filePaths = { "C:/Users/luke/Documents/logs/寻秦OL" };
		for (String filePath : filePaths) {
			for (int i = 0; i < 1; i++) {
				List<? extends CommonRecharge> list = ParseXlsxFile.read(filePath, "4002001032", Recharge寻秦OL.class,
				        "UTF-8", i);
