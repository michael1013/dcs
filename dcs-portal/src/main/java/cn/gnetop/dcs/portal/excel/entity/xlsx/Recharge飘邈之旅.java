...
public class Recharge飘邈之旅 extends CommonRecharge {

	private static final long serialVersionUID = -3355539330629548062L;

	public Recharge飘邈之旅(String gameid, Map<String, Object> map, String file, Integer sheet) {
		if (MapUtils.isNotEmpty(map)) {
			if (map.size() == 6) {
				this.gameid = gameid;
				this.file = file;
				if (StringUtils.isBlank(String.valueOf(map.get(5)))) {
					return;
				}
				this.userid = String.valueOf(map.get(5));
				...
				try {
					this.logTime = DateUtils
					        .formate(DateUtils.parseDate(String.valueOf(map.get(0)), "MM/dd/yyyy HH:mm:ss",
				...
				if (StringUtils.isNoneBlank(this.file)) {
					try {
						this.serverid = this.gameid + '_' + map.get(3);
					} catch (Exception e) {
						e.printStackTrace();
						return;
					}
	...
	public static void main(String[] args) {
		String[] filePaths = { "C:/Users/luke/Documents/logs/飘邈之旅" };
		for (String filePath : filePaths) {
			for (int i = 0; i < 1; i++) {
				List<? extends CommonRecharge> list = ParseXlsxFile.read(filePath, "4002001023", Recharge飘邈之旅.class,
				        "UTF-8", i);
