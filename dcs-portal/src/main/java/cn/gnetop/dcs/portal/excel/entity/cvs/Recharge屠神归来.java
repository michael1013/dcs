...
public class Recharge屠神归来 extends CommonRecharge {
	...
	public Recharge屠神归来(String gameid, String value, String file) {
		String values = value.replaceAll("\"", "");
		String[] column = values.split(",");
		if (column.length == 19) {
			this.userid = column[10];
			// this.username = column[1];
		...
		} else if (column.length == 18) {
			this.userid = column[9];
			// this.username = column[1];
			...
			this.amount = column[10];
			this.currency = "CNY";
			this.channel = column[15];
			this.isValid = true;
			this.gameid = gameid;
			this.serverid = this.gameid + '_' + column[5];
			this.createTime = DateUtils.getDateString();
			this.md5 = MD5Utils.encrypt(this.toString());
		} else {
			this.isValid = false;
	...
	public static void main(String[] args) {
		String[] filePaths = { "C:/Users/luke/Documents/logs/屠龙杀" };
		for (String filePath : filePaths) {
			List<? extends CommonRecharge> list = ParseCsvFile.read(filePath, "4002001029", Recharge屠神归来.class);
