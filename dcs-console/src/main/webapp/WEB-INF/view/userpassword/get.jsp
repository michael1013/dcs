...
<li class="list-group-item">
	...
	<span class="list-group-item-content">${userpassword.id}</span>
	<span class="list-group-item-title">密文:</span>
	<span class="list-group-item-content">${userpassword.pwd}</span>
	<span class="list-group-item-title">修改时间:</span>
	<span class="list-group-item-content">${userpassword.updateTime}</span>
	<span class="list-group-item-title">操作:</span>
	<span class="list-group-item-content" style="width:auto;">
		<span class="glyphicon glyphicon-edit" onclick="reset(${userpassword.id}, ${userpassword.userid})">重置密码</span>
	</span>
</li>
<script>
function reset(id, userid){
	...
	$divCover = CommonUI.element("div", {
		style : "position:fixed;top:0px;left:0px;width:100%;height:100%;background:rgba(0,0,0,0.5);z-index:101;"
	});
...
function resetSub(obj, id){
	$(this).parent().find("input").attr("disabled","disabled");
	var pwd = $("#new-pwd").val();
	$.ajax({
		url:ctx+"/userpassword/update",
		data:{"id":id,"pwd":pwd},
		success:function(r){
			alert(r);
