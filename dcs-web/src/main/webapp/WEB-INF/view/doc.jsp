				...
				<div>
					...
					<div class="auth-announce">
						...
						<div class="json-r" style="padding: 10px;border-box:box-sizing;">
							...
							<p><span class="json-k">authenticateCode</span>: 认证编号由 <span class="json-k">version|componentCode|timestamp</span> 组成, 使用 <span class="json-v">DES+base64</span> 加密算法加密, 加密密钥: 由服务端分配, 请向服务端工作人员索取</p>
						</div>
						...
						<div class="json-r">
							...
							<c:forEach items="${action.columnList}" var="column" varStatus="st">
								<p style="text-indent: 18px;">
									...
									<span class="json-c">//	<c:out value="${column.comment}"></c:out><c:if test="${column.description ne ''}">&nbsp;<span style="color:#e1234a">注:${column.description}</span></c:if></span>
								</p>
							</c:forEach>
							<p>}</p>
						</div>
					...
					<div>
						<ul class="result-code-li" style="font-size: 13px;margin-top: 20px;">
							...
							<c:forEach items="${resultList}" var="result">
								<li>
									<span class="li-col-1">${result.resultCode}</span><span class="li-col-2">${result.resultMsg}</span><span class="li-col-3"></span>
								</li>
							</c:forEach>
						</ul>
					</div>
